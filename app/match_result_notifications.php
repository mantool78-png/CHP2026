...
/**
 * Рассылка уточнения после исправления счёта матча (например, овертайм → основное время).
 *
 * @return array{match_id:int,recipients:int,sent:int,skipped:int,failed:int,error?:string}
 */
function run_match_result_correction_notifications(int $matchId, int $prevHome, int $prevAway): array
{
    $out = [
        'match_id' => $matchId,
        'recipients' => 0,
    ...
    foreach ($recipients as $recipient) {
        $userId = (int) $recipient['id'];
        $hadLog = db()->prepare(
            'SELECT result_home, result_away FROM match_result_notification_log WHERE user_id = ? AND match_id = ?'
        );
        $hadLog->execute([$userId, $matchId]);
        $prevLog = $hadLog->fetch() ?: null;

        $touchStmt->execute([$userId, $matchId, $resultHome, $resultAway]);

        $ok = mail_send_match_result_correction(
            (string) $recipient['email'],
            (string) $recipient['name'],
            $match,
            [
        ...
        if ($ok) {
            ...
            if ($prevLog === null) {
                $releaseStmt->execute([$userId, $matchId]);
            } elseif ($prevLog['result_home'] !== null && $prevLog['result_away'] !== null) {
                $touchStmt->execute([
                    $userId,
                    $matchId,
                    (int) $prevLog['result_home'],
                    (int) $prevLog['result_away'],
                ]);
            } else {
                $releaseStmt->execute([$userId, $matchId]);
            }
