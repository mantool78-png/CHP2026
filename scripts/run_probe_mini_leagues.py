#!/usr/bin/env python3
"""Deploy and run mini-leagues probe on production."""
...
def main() -> int:
    ...
    ftp.set_pasv(True)
    with open(ROOT / "public/cron_probe_mini_leagues.php", "rb") as f:
        ftp.storbinary("STOR /public/cron_probe_mini_leagues.php", f)
    chunks: list[bytes] = []
    ...
    url = f"https://wc2026.gymacro.ru/public/cron_probe_mini_leagues.php?{q}"
    body = urllib.request.urlopen(url, timeout=120, context=ssl.create_default_context()).read().decode(
        "utf-8", "replace"
    )
    out = ROOT / "scripts" / "_mini_leagues_probe.txt"
